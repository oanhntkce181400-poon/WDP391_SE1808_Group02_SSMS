 ...
 *  • 45 SV K26 (nhóm SE2601-01) → ClassEnrollment HK1 + classGroup trên ClassSection
 *  • HK1 bắt đầu: 01/03/2026
 *
 *  • Tạo EnrollmentSnapshot HK1 → hiển thị trong "Lịch sử xếp lớp" (không cần bấm Lưu trên FE)
 *
 * Chạy:  node seed-K26-complete.js
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
async function main() {
  ...
  const classCounter  = {}; // _id → increment
  ...
  console.log(`✅ ${enrollmentDocs.length} enrollments created / already exist`);

  // ── 9d. Attendance HK1 ────────────────────────────────────────────────────────
  /** Schedule.dayOfWeek: 1=T2 … 6=T7, 7=CN — khớp với attendance.service (không dùng trực tiếp getDay === 7 cho CN). */
  ...
  // ── EnrollmentSnapshot → trang "Lịch sử xếp lớp" / điểm danh theo snapshot ──
  const SNAPSHOT_TITLE = `K26_SE_2026 — seed HK1 (${CLASS_GROUP_TAG})`;
  const semesterDoc = await Semester.findOne({
    code: `${hk1.academicYear}_${hk1.num}`,
  }).lean();
  const hk1SectionIds = hk1Sections.map((s) => s._id);

  // Dùng k26Students đã upsert ở trên (majorCode='SE', cohort=26)
  const enrollRows = await ClassEnrollment.find({
    classSection: { $in: hk1SectionIds },
    status:       'enrolled',
    student:      { $in: snapStudentIds },
  })
  ...
  for (const row of enrollRows) {
    ...
    const subj = cs && cs.subject;
    logsByStudent.get(sid).enrolled.push({
  ...
  const snapshotLogs = [...logsByStudent.values()].sort((a, b) =>
    String(a.studentCode).localeCompare(String(b.studentCode)),
  );

  if (semesterDoc && snapshotLogs.length > 0) {
    await EnrollmentSnapshot.deleteMany({ title: SNAPSHOT_TITLE });
    await EnrollmentSnapshot.create({
      title: SNAPSHOT_TITLE,
      description:
        'Tự động từ seed-K26-complete.js — ClassEnrollment HK1; có thể xem/sửa tên trên trang Lịch sử xếp lớp.',
      ...
      curriculumCode: [curriculum.code, curriculum.name].filter(Boolean).join(' — '),
      filters: {
        majorCodes:     [MAJOR_CODE],
        enrollmentMode: 'normal',
        source:         'seed-K26-complete',
      },
      dryRun: false,
      summary: {
        totalStudents:           snapshotLogs.length,
        processedStudents:       snapshotLogs.length,
        candidateStudents:       STUDENTS_COUNT,
        studentsWithEnrollments: snapshotLogs.filter((l) => l.enrolled.length).length,
    ...
    console.log(`✅ Enrollment snapshot: "${SNAPSHOT_TITLE}" (Lịch sử xếp lớp)\n`);
  } else if (!semesterDoc) {
    console.warn('⚠️  Không tìm thấy Semester HK1 trong DB — bỏ qua EnrollmentSnapshot.\n');
  } else {
    console.warn('⚠️  Không có ClassEnrollment HK1 — bỏ qua EnrollmentSnapshot.\n');
  }
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    console.log(`║    ${sc}  SV K26 số ${pad(i, 3)}.padEnd(14)  ${sc.toLowerCase()}@fpt.edu.vn         ║`);
  }
