...
async function getCreditsFromCurriculum(curriculumId, semesterOrder) {
  const subjects = await curriculumService.getSubjectsBySemester(curriculumId, semesterOrder);
  let totalCredits = 0;
  const list = [];
 ...
 * @returns {Object|null}
 */
async function findTuitionByCurriculumSemester(cohort, semesterOrder, majorCode) {
...
async function checkSemesterPaymentRequirement(studentId) {
  ...
  const semesterPaymentCode = generateSemesterPaymentCode(curriculumSemesterOrder, curriculumCode);
  ...
  if (curriculum && curriculum._id) {
    const creditsInfo = await getCreditsFromCurriculum(curriculum._id, curriculumSemesterOrder);
  ...
  const tuitionFeePayload = (totalCreditsFromCurriculum > 0 || tuitionFeeRule) ? {
...
async function getAllUnpaidSemesters(studentId) {
  ...
  for (let i = 1; i <= currentCurriculumSemester; i++) {
    if (!paidSemesters.has(i)) {
      const tuitionFee = await findTuitionByCurriculumSemester(student.cohort, i, student.majorCode);
...
async function checkEnrollmentPermission(studentId) {
  ...
  if (paymentStatus.isNewStudent && !paymentStatus.hasPaid) {
    return {
      ...
      message: 'Sinh viên mới phải thanh toán học phí kỳ 1 trước khi đăng ký môn học',
      paymentStatus
    };
  }
